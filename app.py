...
def predict():
    ...
    processed_image_pca = pca.transform(processed_image_flat)  # Transform to 50 features
    ...
    for model_name, model in models.items():
        if model_name == 'cnn':
            ...
            prediction = model.predict_proba(processed_image_pca) if hasattr(model, 'predict_proba') else model.predict(processed_image_pca)
            confidence_score = float(np.max(prediction)) if hasattr(model, 'predict_proba') else 1.0
            predicted_class = int(np.argmax(prediction)) if hasattr(model, 'predict_proba') else int(prediction[0])
